...
if __name__ == '__main__':
    ...
    parser.add_argument('--root_path', type=str, default='./workdir/ml-1m', help='Directory containing seq-pat-pair.pth and pre-trained_embedding.ckpt.')
    ...
    parser.add_argument('--batch_size', type=int, default=256, help='Training epochs of the regenerator.')
    ...
    num_item_dict = {
        ...
        'books': tmp_saved['parameters']['item_embedding.weight'].shape[0],
    ...
    SOS = num_item
    EOS = num_item + 1
    # Allocate token for domains
    first_domain_token = EOS + 1
    

    source, target = [], []
    source_seqlen, target_seqlen = [], []
    all_different_elements = set()
    id__=1
    for example in data:
        s, t, s_domain, t_domain = example  # Unpack data
        # 将两个列表转换为集合
        set_s = set(s_domain)
        set_t = set(t_domain)

        # 将当前样例的不同元素更新到总体集合
        all_different_elements.update(set_s)
        all_different_elements.update(set_t)

        s_domain_token = [first_domain_token + i for i in s_domain]
        t_domain_token = [first_domain_token + i for i in t_domain]

        # Create source sequence: item-domain-item-domain format
        source_sequence = [item for pair in zip(s, s_domain_token) for item in pair]

        # Create target sequence: item-domain-item-domain format
        target_sequence = [item for pair in zip(t, t_domain_token) for item in pair]
        if (id__ == 1):
        ...
        source_seqlen.append(len(source_sequence) + 2)
        target_seqlen.append(len(target_sequence) + 2)

        # Add SOS and EOS tokens
        s_tensor = torch.tensor([SOS] + source_sequence + [EOS])
        t_tensor = torch.tensor([SOS] + target_sequence + [EOS])

        # Append to lists
        source.append(s_tensor)
        target.append(t_tensor)
    domain_count=len(all_different_elements)
    ...
    # Pad target to a fixed length, if needed
    ...
    class Generator(nn.Module):
        ...
        def load_pretrained(self):
            ...
            print(saved['parameters']['item_embedding.weight'].shape)
            pretrained = saved['parameters']['item_embedding.weight']
            #pretrained = nn.Embedding(num_item_dict[dataset_name], 64).weight.data
            # 使用 nn.Embedding 来创建随机初始化的嵌入
            random_embedding = nn.Embedding(domain_count, 64)
            random_initialized = random_embedding.weight.data
            ...
            # 创建Embedding层
            self.item_embedding = nn.Embedding.from_pretrained(combined, padding_idx=0, freeze=False)
            #print("@@item_embedding",self.item_embedding.weight.shape)
            self.item_embedding_decoder = self.item_embedding

            # 保存合并后的嵌入到新的文件中
            new_saved = {
                'parameters': {
                    'item_embedding.weight': self.item_embedding.weight.detach().cpu()
                }
            }
            new_path = os.path.join(args.root_path, 'pre-trained_embedding-new.ckpt')
            torch.save(new_saved, new_path)
        ...
        def forward(self, src, tgt, src_mask, tgt_mask,
            ...
            #print('@@@@@@',self.item_embedding(src).shape,src_position_embedding.shape)
            #print('@@@@@@',(self.item_embedding(src)+src_position_embedding).shape)
