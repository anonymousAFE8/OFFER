...
def _normalize_device(device):
    ...
    if isinstance(device, int):
        ...
        visible_ids = [gpu.strip() for gpu in visible.split(',') if gpu.strip()]
        if visible_ids and (device < 0 or device >= len(visible_ids)):
            return 'cuda:0'
...
def set_device(config):
    ...
    config['device'] = _normalize_device(config['device'])
...
def load_config(config : dict):
    ...
    if 'train' in config['data']:
        for category_k, category_v in config['data']['train'].items():
            config['train'][category_k] = category_v
    return config
...
class Hypergrad:
    ...
    This implementation follows the standard implicit-differentiation algorithm,
    with small practical differences.
