...
class BaseDataset(Dataset):
    ...
    def unpack(self, to_be_unpacked):
        normalized_rows = [self._normalize_row(_) for _ in to_be_unpacked]
        user_id = torch.tensor([_[0] for _ in normalized_rows], device=self.device)
        user_seq = torch.tensor([_[1] for _ in normalized_rows], device=self.device)
        target_item = torch.tensor([_[2] for _ in normalized_rows], device=self.device)
        seq_len = torch.tensor([_[3] for _ in normalized_rows], device=self.device)
        label = torch.tensor([_[4] for _ in normalized_rows], device=self.device)
        domain_id = torch.tensor([_[5] for _ in normalized_rows], device=self.device)
        if self.phase != 'train':
            user_hist = user_seq
    ...
    def _fit_sequence_length(self, value, pad_value=0):
        ...
        return seq + [pad_value] * (self.max_seq_len - cur_len)

    def _normalize_row(self, row):
        row = list(row)
        row[1] = self._fit_sequence_length(row[1])
        row[2] = self._fit_sequence_length(row[2])
        row[3] = min(int(row[3]), self.max_seq_len)
        row[4] = self._fit_sequence_length(row[4])
        row[5] = self._fit_sequence_length(row[5])
        return row
