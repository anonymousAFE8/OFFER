...
class DataProcessor:
    ...
    def to_seq_data(
        ...
        ratings_data_transformed.ratios = ratings_data_transformed.ratios.apply(
            lambda x: ",".join([str(v) for v in x])
        )
...
class KuaiRand1KDataProcessor(DataProcessor):
    def __init__(
        self,
        # download_path: str,
        # saved_name: str,
...
class CIKMDataProcessor(DataProcessor):
    def __init__(
        self,
        prefix: str,
        input_file_path: str,
    ...
    def preprocess_rating(self) -> int:
        df = pd.read_csv(self._input_file_path, sep=',', header=None)
        print(df.head())
        df.columns = ['user_id', 'item_id', 'behavior', 'timestamps']   # 不使用额外特征

        behavior_mapping = {'pv': 0, 'fav': 1, 'cart': 2, 'buy': 3}
        df['behavior'] = df['behavior'].map(behavior_mapping)

        if self._convert_timestamp:
            df["timestamps"] = pd.to_datetime(
                df["timestamps"], unit="s"
            )
        ...
        df_sorted = df.sort_values(by=["timestamps"]).groupby("user_id")
        df_seq = pd.DataFrame(
            data={
                "user_id": list(df_sorted.groups.keys()),
                "item_ids": list(df_sorted.item_id.apply(list)),
                "ratings": list(df_sorted.behavior.apply(list)),
                "timestamps": list(df_sorted.timestamps.apply(list)),
            }
        )

        df_seq["seq_len"] = df_seq["item_ids"].apply(len)
        df_seq = df_seq[df_seq["seq_len"] >= self._min_seq_length]
        df_seq.drop(columns=["seq_len"], inplace=True)

        num_unique_users = len(set(df_seq["user_id"].values))
        num_unique_items = len(set(list(chain.from_iterable(df_seq["item_ids"]))))

        # 在这里不区分训练集和测试集，统一存储
        seq_ratings_data = self.to_seq_data(df_seq, None)
        seq_ratings_data.sample(frac=1).reset_index().to_csv(
            self.output_format_csv(), index=False, sep=","
        )
        
        print(f"{self._prefix} num_unique_users:{num_unique_users}")
        print(f"{self._prefix} num_unique_items:{num_unique_items}")
        return num_unique_items


class MusicDataProcessor(DataProcessor):
    def __init__(
        self,
        prefix: str,
        input_file_path: str,
    ...
    def map_playback_bucket(self, rate):
        if pd.isna(rate):
            return 1  # unknown
        elif rate < 10:
            return 2  # drop
    ...
    def preprocess_rating(self) -> int:
        df = pd.read_csv(self._input_file_path, sep=',', 
                         usecols=['uid', 'timestamp', 'item_id', 'played_ratio_pct', 'event_type'])
        print(df.head())
        # df.columns = ['uid', 'item_id', 'event_type', 'timestamp', 'played_ratio_pct']   # 不使用额外特征

        behavior_mapping = {'dislike': 0, 'undislike': 1, 'listen': 2, 'unlike': 3, 'like': 4}
        df['event_type'] = df['event_type'].map(behavior_mapping)

        df['played_ratio_pct'] = df['played_ratio_pct'].apply(self.map_playback_bucket)
        ...
        df_sorted = df.sort_values(by=["timestamp"]).groupby("uid")
        df_seq = pd.DataFrame(
            data={
                "user_id": list(df_sorted.groups.keys()),
                "item_ids": list(df_sorted.item_id.apply(list)),
                "ratios": list(df_sorted.played_ratio_pct.apply(list)),
                "ratings": list(df_sorted.event_type.apply(list)),
                "timestamps": list(df_sorted.timestamp.apply(list)),
            }
        )
...
def get_common_preprocessors() -> (
    Dict[
        ...
        Union[
            ...
            CIKMDataProcessor, MusicDataProcessor
        ],
    ]
):
    ...
    """KuaiRand1K_dp = KuaiRand1KDataProcessor(
        prefix="KuaiRand1K"
    )"""
    cikm_dp = CIKMDataProcessor(
        prefix="cikm",
        input_file_path="/home/rzhou/iclr/CIKM/user_behavior_train.csv",
        min_seq_length=20,
        convert_timestamp=False,
        expected_num_unique_items=3737409,
    )
    music_dp = MusicDataProcessor(
        prefix="music",
        input_file_path="/home/rzhou/iclr/rank_code/your_file.csv",
        min_seq_length=0,
        convert_timestamp=False,
        expected_num_unique_items=934057,
    )
    return {
        "ml-1m": ml_1m_dp,
        "ml-20m": ml_20m_dp,
