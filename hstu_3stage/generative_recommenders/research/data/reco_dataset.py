...
from generative_recommenders.research.data.dataset import DatasetV2, MultiFileDatasetV2, MusicDatasetV2
...
def get_reco_dataset(
    ...
    if dataset_name == "ml-1m":
    ...
    elif dataset_name == "KuaiRand1K":
        train_dataset = DatasetV2(
            ratings_file="/home/rzhou/iclr/code/tmp/KuaiRand1K/sasrec_format_by_user_train.csv",
            padding_length=max_sequence_length + 1,  # target
            ignore_last_n=1,
            shift_id_by=1,  # [0..n-1] -> [1..n]
            chronological=chronological,
        ...
        eval_dataset = DatasetV2(
            ratings_file="/home/rzhou/iclr/code/tmp/KuaiRand1K/sasrec_format_by_user_test.csv",
            padding_length=max_sequence_length + 1,  # target
            ignore_last_n=0,
            shift_id_by=1,  # [0..n-1] -> [1..n]
            chronological=chronological,
        ...
        dp = get_common_preprocessors()[dataset_name]
        print(dp)
        train_dataset = MusicDatasetV2(
            ratings_file=dp.output_format_csv(),
            padding_length=max_sequence_length + 1,  # target
            ignore_last_n=0,
            chronological=chronological,
        )
        eval_dataset = MusicDatasetV2(
            ratings_file=dp.output_format_csv(),
            padding_length=max_sequence_length + 1,  # target
            ignore_last_n=0,
            chronological=chronological,
    ...
    if dataset_name == "ml-1m" or dataset_name == "ml-20m":
        ...
        item_features = None
        max_item_id = 4369953       # handle.py中统计得到
        all_item_ids = [x + 1 for x in range(max_item_id)]  # pyre-ignore [6]
    elif dataset_name == "music":
        item_features = None
        max_item_id = dp.expected_num_unique_items()
        print(f"zr===dp.expected_num_unique_items():{dp.expected_num_unique_items()}")
        all_item_ids = [x + 1 for x in range(max_item_id)]  # pyre-ignore [6]
    else:
