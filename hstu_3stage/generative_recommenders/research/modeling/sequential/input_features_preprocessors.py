...
class LearnablePositionalEmbeddingInputFeaturesPreprocessor(
    ...
    def __init__(
        ...
        # 这里在前向函数中会对有效位置的action加一，所以需要的编码数量为action类别加一，0是padding
        if exp_conf_dict.get("add_action", False) == True:
            self._action_emb = torch.nn.Embedding(
                exp_conf_dict["num_actions"] + 1, self._embedding_dim, padding_idx=0
            )
    ...
    def forward(
        ...
        if self.exp_conf_dict.get("use_user_id", False) == True:
            user_embeddings = past_embeddings * (self._embedding_dim**0.5) + torch.cat([
                torch.zeros(B, 1, D).to(past_ids.device),
                self._pos_emb(torch.arange( N - 1, device=past_ids.device).unsqueeze(0).repeat(B, 1))
                ], dim = 1)
        else:
        ...
        if self.exp_conf_dict.get("add_action", False) == True:
            actions = past_payloads["ratings"]
            # 因为actions的padding和部分行为都是0，所以对有效位的padding需要加1
            actions += (past_ids != 0).int()
            if self.exp_conf_dict.get("use_user_id", False) == True:
                user_embeddings = user_embeddings + torch.cat([
                    torch.zeros(B, 1, D).to(past_ids.device),
                    self._action_emb(actions),
                   ], dim = 1)
            else:
                user_embeddings = user_embeddings + self._action_emb(actions)
        ...
        if self.exp_conf_dict.get("use_user_id", False) == True:
            if self.exp_conf_dict.get("use_action", False) == True:
                valid_mask = torch.repeat_interleave(valid_mask, repeats=2, dim=1)
            valid_mask = torch.cat([torch.ones(B, 1, 1).float().to(past_ids.device), valid_mask], dim=1)
