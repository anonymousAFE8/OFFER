...
# 包含特征的负样本采集方案
class FeatNegativesSampler(NegativesSampler):
    def __init__(
        self,
        num_items,
        all_item_ids,
    ...
    def forward(
        ...
        sampled_ids = self._all_item_ids[sampled_offsets.view(-1)].reshape(output_shape)
        sampled_feats, feat_names = self.get_feat(sampled_ids.view(-1).cpu(), "item", re_feat_name= True)
        for name in feat_names:
           #print(sampled_feats[name])
           sampled_feats[name] = torch.tensor(sampled_feats[name], dtype=torch.int64).to(positive_ids.device).reshape(output_shape[0], output_shape[1], -1).squeeze(-1)
        sampled_embeddings = self.feat_emb(item_ids = sampled_ids, item_actions=None, user_id=None, feat = sampled_feats)
