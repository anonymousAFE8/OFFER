...
class RelativeBucketedTimeAndPositionBasedBias(RelativeAttentionBiasModule):
    ...
    def forward(
        ...
        if self.exp_conf_dict.get("use_action", False):
            rel_ts_bias = rel_ts_bias.view(B, N//2, N//2)
            rel_ts_bias = torch.repeat_interleave( torch.repeat_interleave(rel_ts_bias, repeats=2, dim=1), repeats=2, dim=2)
        else:
            rel_ts_bias = rel_ts_bias.view(B, N, N)
...
class HSTU(SequentialEncoderWithLearnedSimilarityModule):
    ...
    def generate_user_embeddings(
        ...
        user_embeddings = self.generate_sequence_embeddings(
            past_lengths=past_lengths,
            past_ids=past_ids,
            past_embeddings=past_embeddings,
            past_payloads=past_payloads,
    ...
    def encode_hidden(
        ...
        encoded_seq_embeddings = self.generate_sequence_embeddings(
            past_lengths=past_lengths,
            past_ids=past_ids,
            past_embeddings=past_embeddings,
            past_payloads=past_payloads,
